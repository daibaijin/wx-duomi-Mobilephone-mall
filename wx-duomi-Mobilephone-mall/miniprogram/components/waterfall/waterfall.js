...
Page({
    ...
    loadImgFinish(e) {
        ...
        // 获取所有图片容器的节点信息
        dom.select("#waterfall-item-id-" + index).fields({
            size: true
        }, (res) => {
            that.setData({
                num: that.data.num + 1,
            ...
            if (that.data.num == that.data.waterfallarr.length) {
                for (let i = that.data.oldNum; i < that.data.num; i++) {
                    ...
                    // 瀑布流懒加载
                    that.createIntersectionObserver().relativeToViewport({
                        bottom: 50
                    }).observe('#waterfall-item-id-' + i, (res) => {
                ...
                // 瀑布流加载完成后，通知父组件加载完毕
                that.triggerEvent('waterFallResh', {
                    loading: true
                })
            }
        }).exec()
